...
namespace DRLoader
{
    ...
    /// 一个资源目录的管理类
    /// </summary>
    public class ResDir:IDisposable
    {
        public static ResDir Create(string path)
        {
...
#region HEAD
        ...
        /// <returns></returns>
        public bool Exist(string resName )
        {
        ...
        public void BeginAddRes(string resName)
        {
            ...
            _AddingRes = resName;
            _Saving = new FileStream(path, FileMode.Create, FileAccess.Write);
        }
        /// <summary>
        /// 处理添加数据到资源
        /// </summary>
        ...
        public void EndAddRes(string resName, bool cancelAdd )
        {
            ...
            if( cancelAdd )
            {
                ...
                //UnityEngine.Debug.LogWarning("取消添加资源，资源被删除："+resName);
            }
            else
            {
                _AllRes.Add(resName);
        ...
        public void AddRes(string resName, string srcPath )
        {
            ...
            byte[] data = new byte[10 * 1024 * 1024];//10m缓存
