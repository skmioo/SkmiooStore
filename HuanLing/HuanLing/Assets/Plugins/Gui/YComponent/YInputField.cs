...
namespace UnityEngine.UI
{
    ...
    /// </summary>
    [CustomLuaClass]
    [AddComponentMenu("YUI/YInput Field", 31)]
    public class YInputField
        : YSelectable,
        ...
        ICanvasElement
    {
        ...
        protected YLabel m_TextComponent;
        ...
        [SerializeField]
        private Color m_SelectionColor = UIResDefine.InputSelectColor;
        ...
        public bool shouldHideMobileInput
        {
            set
            {
                YSetPropertyUtility.SetStruct(ref m_HideMobileInput, value);
            }
            get
            {
        ...
        public float caretBlinkRate
        {
            ...
            set
            {
                if (YSetPropertyUtility.SetStruct(ref m_CaretBlinkRate, value))
                {
                    if (m_AllowInput)
                        SetCaretActive();
                }
        ...
        public YLabel textComponent { get { return m_TextComponent; } set { YSetPropertyUtility.SetClass(ref m_TextComponent, value); } }

        public Graphic placeholder { get { return m_Placeholder; } set { YSetPropertyUtility.SetClass(ref m_Placeholder, value); } }

        public Color selectionColor { get { return m_SelectionColor; } set { YSetPropertyUtility.SetColor(ref m_SelectionColor, value); } }

        public SubmitEvent onEndEdit { get { return m_EndEdit; } set { YSetPropertyUtility.SetClass(ref m_EndEdit, value); } }

        public OnChangeEvent onValueChange { get { return m_OnValueChange; } set { YSetPropertyUtility.SetClass(ref m_OnValueChange, value); } }

        public OnValidateInput onValidateInput { get { return m_OnValidateInput; } set { YSetPropertyUtility.SetClass(ref m_OnValidateInput, value); } }

        public int characterLimit { get { return m_CharacterLimit; } set { YSetPropertyUtility.SetStruct(ref m_CharacterLimit, value); } }

        // Content Type related

        public ContentType contentType { get { return m_ContentType; } set { if (YSetPropertyUtility.SetStruct(ref m_ContentType, value)) EnforceContentType(); } }

        public LineType lineType { get { return m_LineType; } set { if (YSetPropertyUtility.SetStruct(ref m_LineType, value)) SetToCustomIfContentTypeIsNot(ContentType.Standard, ContentType.Autocorrected); } }

        public InputType inputType { get { return m_InputType; } set { if (YSetPropertyUtility.SetStruct(ref m_InputType, value)) SetToCustom(); } }

        public TouchScreenKeyboardType keyboardType { get { return m_KeyboardType; } set { if (YSetPropertyUtility.SetStruct(ref m_KeyboardType, value)) SetToCustom(); } }

        public CharacterValidation characterValidation { get { return m_CharacterValidation; } set { if (YSetPropertyUtility.SetStruct(ref m_CharacterValidation, value)) SetToCustom(); } }

        // Derived property
        public bool multiLine { get { return m_LineType == LineType.MultiLineNewline || lineType == LineType.MultiLineSubmit; } }
        // Not shown in Inspector.
        public char asteriskChar { get { return m_AsteriskChar; } set { YSetPropertyUtility.SetStruct(ref m_AsteriskChar, value); } }
        ...
        {
            SelectAll();
            if (m_BeforEditLua != null)
            {
                m_BeforEditHandler(m_BeforEditLua, this, m_Text);
            }
        }
        ...
        public virtual void OnUpdateSelected(BaseEventData eventData)
        {
            ...
            while (Event.PopEvent(m_ProcessingEvent))
            {
                if (m_ProcessingEvent.rawType == EventType.KeyDown)
                {
                    ...
                    {
                        DeactivateInputField();
                        if (m_Lua != null && m_LuaReturnHandler != null)
                        {
                            m_LuaReturnHandler(m_Lua, this);
                        }
                        break;
                    }
                }
        ...
        {   
            if (m_submitLua != null  && m_LuaSubmitHandler != null)
            {
                m_LuaSubmitHandler(m_submitLua, this, m_Text);
            }
        ...
        {
            if (cachedInputTextGenerator.lines.Count <= 0)
                return;
        ...
        protected char Validate(string text, int pos, char ch)
        {
            ...
            if (characterValidation == CharacterValidation.Integer || characterValidation == CharacterValidation.Decimal)
            {
                ...
                //无符号整型
            else if (characterValidation == CharacterValidation.UnsignedInt)
            {
                //如果即将输入的字符位置为1，那个不允许输入0
                bool firstPositoinIsZero = false;
                if (ch == '0')
                {
                    firstPositoinIsZero = (pos == 0) || (hasSelection && (caretPosition == 0 || caretPosition == text.Length && selectionAnchorPosition == 0));
                }
                if (!firstPositoinIsZero)
                {
                    if (ch >= '0' && ch <= '9') return ch;
                }
            }
        ...
        public override void OnDeselect(BaseEventData eventData)
        {
            ...
            if (m_disSelect != null)
            {
                m_disHandle(m_disSelect, m_disSelectArgs);
            }
        ...
        private void EnforceContentType()
        {
            switch (contentType)
            {
                ...
                case ContentType.UnsignedInt:
                    {
                        m_LineType = LineType.SingleLine;
                        m_InputType = InputType.Standard;
                        m_KeyboardType = TouchScreenKeyboardType.NumberPad;
                        m_CharacterValidation = CharacterValidation.UnsignedInt;
                        return;
                    }
                default:
                    {
        ...
        private LuaTable m_Lua;
        private Action<LuaTable, YInputField> m_LuaReturnHandler;

        private LuaTable m_submitLua;
        private Action<LuaTable, YInputField, string> m_LuaSubmitHandler;

        private LuaTable m_BeforEditLua;
        private Action<LuaTable, YInputField, string> m_BeforEditHandler;
        /// <summary>
        /// 设置回车键回调
        /// </summary>
        /// <param name="lua"></param>
        /// <param name="handler"></param>
        public void SetReturnHandler(LuaTable lua, Action<LuaTable, YInputField> handler)
        {
            m_Lua = lua;
            m_LuaReturnHandler = handler;
        }

        public void SetSubmitHandler(LuaTable lua, Action<LuaTable, YInputField, string> handler)
        {
            m_submitLua = lua;
            m_LuaSubmitHandler = handler;
        }

        public void BeforeEditHanler(LuaTable lua, Action<LuaTable, YInputField, string> handler)
        {
            m_BeforEditLua = lua;
            m_BeforEditHandler = handler;
        }

        private LuaTable m_valueLua;
        private Action<LuaTable, string, int> m_valueHandle;
        /// <summary>
        /// inputfiled.text 发生变化的回调
        /// </summary>
        /// <param name="lua"></param>
        /// <param name="handler"></param>
        public void ValueChangeHandle(LuaTable lua, Action<LuaTable, string, int> handler)
        {
            m_valueLua = lua;
            m_valueHandle = handler;
            m_OnValueChange.AddListener(Valuehandle);
        }
        /// <summary>
        /// 处理函数，用了一个timer，防止频繁的发生通知 暂时不用试试看
        /// </summary>
        /// <param name="text"></param>
        private void Valuehandle(string text)
        {
            m_valueHandle(m_valueLua, text, text.Length);
        }

        private LuaTable m_endEditLua;
        private Action<LuaTable, string, int> m_endEdithandle;
        /// <summary>
        /// 输入结束的回调
        /// </summary>
        /// <param name="lua"></param>
        /// <param name="handle"></param>
        public void EndEditHandle(LuaTable lua, Action<LuaTable, string, int> handle)
        {
            m_endEditLua = lua;
            m_endEdithandle = handle;
            onEndEdit.AddListener(_EndEditHandle);
        }

        private void _EndEditHandle(string text)
        {
            if  ( m_endEdithandle != null )
                m_endEdithandle(m_endEditLua, text, text.Length);
        }

        private LuaTable m_disSelect;
        private LuaTable m_disSelectArgs;
        private Action<LuaTable, LuaTable> m_disHandle;
        /// <summary>
        /// 失去焦点的时候的回调处理
        /// </summary>
        public void DisSelectHandle(LuaTable lua, Action<LuaTable, LuaTable> handle, LuaTable args )
        {
            m_disSelect = lua;
            m_disSelectArgs = args;
            m_disHandle = handle;
        }
        public void DisSelectHandle(LuaTable lua, Action<LuaTable, LuaTable> handle)
        {
            m_disSelect = lua;
            m_disHandle = handle;
        }
        ...
        {
            m_disSelect = null;
            m_disSelectArgs = null;
            m_disHandle = null;
            m_endEditLua = null;
            m_endEdithandle = null;
            m_valueLua = null;
            m_valueHandle = null;
            m_Lua = null;
            m_LuaReturnHandler = null;
            m_submitLua = null;
            m_LuaSubmitHandler = null;
            m_BeforEditLua = null;
            m_BeforEditHandler = null;
        }
    }
}
