...
namespace UnityEngine.UI
{
    ...
    /// </summary>
    [CustomLuaClass]
    [AddComponentMenu("YUI/YImage", 2)]
    [Serializable]
    public class YImage
     : YMaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter, ISpriteUser
    {
        public enum Type
        {
            Simple,
        ...
        public Sprite sprite { get { return m_Sprite; } set { if (YSetPropertyUtility.SetClass(ref m_Sprite, value)) SetAllDirty(); } }

        [NonSerialized]
        private Sprite m_OverrideSprite;
        public Sprite overrideSprite { get { return m_OverrideSprite == null ? sprite : m_OverrideSprite; } set { if (YSetPropertyUtility.SetClass(ref m_OverrideSprite, value)) SetAllDirty(); } }

        /// How the Image is drawn.
        [SerializeField]
        private Type m_Type = Type.Simple;
        public Type type { get { return m_Type; } set { if (YSetPropertyUtility.SetStruct(ref m_Type, value)) SetVerticesDirty(); } }

        [SerializeField]
        private bool m_PreserveAspect = false;
        public bool preserveAspect { get { return m_PreserveAspect; } set { if (YSetPropertyUtility.SetStruct(ref m_PreserveAspect, value)) SetVerticesDirty(); } }

        [SerializeField]
        private bool m_FillCenter = true;
        public bool fillCenter { get { return m_FillCenter; } set { if (YSetPropertyUtility.SetStruct(ref m_FillCenter, value)) SetVerticesDirty(); } }

        /// Filling method for filled sprites.
        [SerializeField]
        private FillMethod m_FillMethod = FillMethod.Radial360;
        public FillMethod fillMethod { get { return m_FillMethod; } set { if (YSetPropertyUtility.SetStruct(ref m_FillMethod, value)) { SetVerticesDirty(); m_FillOrigin = 0; } } }

        /// Amount of the Image shown. 0-1 range with 0 being nothing shown, and 1 being the full Image.
        [Range(0, 1)]
        [SerializeField]
        private float m_FillAmount = 1.0f;
        public float fillAmount { get { return m_FillAmount; } set { if (YSetPropertyUtility.SetStruct(ref m_FillAmount, Mathf.Clamp01(value))) SetVerticesDirty(); } }

        /// Whether the Image should be filled clockwise (true) or counter-clockwise (false).
        [SerializeField]
        private bool m_FillClockwise = true;
        public bool fillClockwise { get { return m_FillClockwise; } set { if (YSetPropertyUtility.SetStruct(ref m_FillClockwise, value)) SetVerticesDirty(); } }

        /// Controls the origin point of the Fill process. Value means different things with each fill method.
        [SerializeField]
        private int m_FillOrigin;
        public int fillOrigin { get { return m_FillOrigin; } set { if (YSetPropertyUtility.SetStruct(ref m_FillOrigin, value)) SetVerticesDirty(); } }
        ...
        [SerializeField]
        private ColorEffectMode m_ColorEff = ColorEffectMode.Default;
        public ColorEffectMode colorEff
        {
            get { return m_ColorEff; }
            set
            {
                if (YSetPropertyUtility.SetStruct(ref m_ColorEff, value))
                {
                    RefreshMaterial();
                }
            }
        ...
        [SerializeField]
        private SpecialEffectType m_SpecialEffectType = SpecialEffectType.None;

        public SpecialEffectType specEffect
        {
            get
            {
                return m_SpecialEffectType;
            }
            set
            {
                if (YSetPropertyUtility.SetStruct(ref m_SpecialEffectType, value))
                {
                    RefreshMaterial();
                }
            }
        ...
        public float pixelsPerUnit
        {
            get
            {
                ...
                float referencePixelsPerUnit = 1;
                if (canvas)
        ...
        public override void OnRebuildRequested()
        {
            ...
            {
                _ApplyImage(isActiveAndEnabled);
            }
        }
#endif
        ...
        // imageID 图片资源编号
        [SerializeField]
        private ResID m_ImageID = ResID.zero;
        private ISpriteHandle _SpriteHandle = null;
        private bool _NativeSized = false;
        bool m_Gray = false;
        ...
        {
            base.Start();
            usedForMask = GetComponent<Mask>() != null;
        }
        ...
        public void SetImageID(int pack, int grp, int res)
        {
            imageID = new ResID(pack, grp, res);
        }

        private enum ImageUIState
        {
            Disabled,
            Enabled,
            Auto
        }
        private void _ApplyImage(bool valid)
        {
            if (valid)
            {//启用
                if (_SpriteHandle != null)
                {
                    if (_SpriteHandle.id == m_ImageID)
                    {//图片一致
                        return;
                    }
                    material = null;
                ...
                {//运行模式下
                    if (m_ImageID.valid)
                    {//加载图片
                        _SpriteHandle = PlayInterface.LoadSprite(m_ImageID, _HandleSpriteLoaded);
                    }
                }
                else
                {//编辑器状态下处理,直接同步加载
                    if (m_ImageID.valid)
                    {//直接加载
                        overrideSprite = PlayInterface.GetResInEditor(m_ImageID, typeof(Sprite)) as Sprite;
                        SetNativeSizeAsyn();
                        RefreshMaterial();
                    }
                    else
        ...
        {
            _SpriteHandle = h;
            overrideSprite = _SpriteHandle.sprite;
            SetNativeSizeAsyn();
            RefreshMaterial();
        }
        private bool SpriteReady(ISpriteHandle h)
        {
        ...
        public void RefreshMaterial()
        {
            ...
            if (specEffect == SpecialEffectType.None)
            {
                if (m_Gray)
                {//变灰色
                    material = SpriteReady(_SpriteHandle) ? _SpriteHandle.grayMat : PlayInterface.GetUIGrayMat();
                }
                else
                {//普通
                    switch (m_ColorEff)
                    {
                        case ColorEffectMode.Overlay:
                            material = SpriteReady(_SpriteHandle) ? _SpriteHandle.overlayMat : PlayInterface.GetUIOverlayMat();
                            break;
                        case ColorEffectMode.Add:
                            material = SpriteReady(_SpriteHandle) ? _SpriteHandle.addMat : PlayInterface.GetUIAddMat();
                            break;
                        default:
                            material = SpriteReady(_SpriteHandle) ? _SpriteHandle.normalMat : PlayInterface.GetUINormalMat();
                            break;
                    }
                }
            }
            else
            {
                switch (specEffect)
                {
                    case SpecialEffectType.BlackFocus:
                        material = PlayInterface.GetUIBlackFocusMat();
                        break;
                    default:
                        material = SpriteReady(_SpriteHandle) ? _SpriteHandle.normalMat : PlayInterface.GetUINormalMat();
                        break;
        ...
        /// <summary>
        /// 是否精确包含点
        /// 使用此方法texture资源必须设置为 Read/Write Enabled
        /// </summary>
        /// <param name="wordPosition">世界坐标</param>
        /// <returns>aphla > 0.1 时返回true，否则false</returns>
        public bool ExactContain(Vector3 wordPosition)
        {
            ...
            var imagePos = transform.position;
            var size = rectTransform.sizeDelta;
            imagePos.x = imagePos.x - (size.x * rectTransform.pivot.x);
            imagePos.y = imagePos.y - (size.y * rectTransform.pivot.y);
            var pos = new Vector2(wordPosition.x - imagePos.x, wordPosition.y - imagePos.y);
            ...
            {
                var subrect = sp.rect;
                var textureWith = subrect.width;
                var textureHeight = subrect.height;
                pos.x += subrect.x;
                pos.y += subrect.y;
                var pixelPos = new Vector2(pos.x * textureWith / size.x, pos.y * textureHeight / size.y);
                var pixelColor = sp.texture.GetPixel((int)pixelPos.x, (int)pixelPos.y);
                return pixelColor.a > 0.1;
            }
            catch (Exception e)
            {
#if UNITY_EDITOR
                Debug.LogErrorFormat("YImage's ExactContain Error: {0}", e);
#endif
            }
            return true;
        }
