...
namespace Skmioo.Net
{
	...
	/// http/https单线程下载器
	/// </summary>
	public class MyHttpLoader : IMyNetLoader
	{
		class HttpItem
		{
			public string[] urls;
			...
			public Action<IMyNetResult> callback;
			public void Clear()
			{
				urls = null;
				form = null;
		...
		HttpItem _Loading;
		MyNetLoadState _Step = MyNetLoadState.Requesting;
		MyHttpRequest _Request;
		...
		public void Load(string[] url, string savePath, Action<IMyNetResult> complete, MyPostForm postForm = null)
		{
		...
		void LoadNext()
		{
			...
			if (_Request == null)
			{
				_Request = new MyHttpRequest();
			}
			_Save = new MyHttpSave(_Loading.savePath, _Loading.breadkPoint);
			TrySrc(false);
		}
		...
		void TrySrc(bool next)
		{
			...
			{
			 //还有其它源可尝试
				_Step = MyNetLoadState.Requesting;
		...
		void HandleRequestReturn(bool success, string error)
		{
			...
			if (success)
			{
				...
				{
					_Step = MyNetLoadState.Transfering;
				}
				else
		...
		public void Update()
		{
			switch (_Step)
			{
				case MyNetLoadState.Requesting:
					if (_Request != null)
					{
						_Request.Update();
					}
					break;
				case MyNetLoadState.Transfering:
	...
	public struct HttpResult : IMyNetResult
	{
		/// <summary>
		/// 下载是否成功
		/// </summary>
