...
namespace Skmioo.Net
{
	...
	/// 一个字节流，提供反序列化，序列化操作
	/// </summary>
	public class MyUnityByteBuff
	{
		byte[] mBuff;
		int mPos;
		int mSize;
		public MyUnityByteBuff(int cap = 256)
		{
			mBuff = new byte[cap];
			mPos = 0;
			mSize = 0;
		...
		public MyUnityByteBuff Clone()
		{
			MyUnityByteBuff ba = new MyUnityByteBuff(mSize);
		...
		public int ReadBytes(byte[] dstBuff, int dstOffset, int len)
		{
			len = len <= 0 ? bytesAvailable : len;
			CheckAvaliable(len);
			Buffer.BlockCopy(mBuff, mPos, dstBuff, dstOffset, len);
			mPos += len;
			return len;
		}

		public int ReadBytes(MyUnityByteBuff dstBuf, int len)
		{
			len = len <= 0 ? bytesAvailable : len;
			CheckAvaliable(len);
		...
		public int WriteBytes(MyUnityByteBuff buff, int srcOffset, int len)
		{
		...
		private void Alloc(int size)
		{
			...
			{
				size = size > 1024 * 1024 ? size : (int)(size * 1.5f);
				byte[] newBuff = new byte[size];
				Buffer.BlockCopy(mBuff, 0, newBuff, 0, mSize);
				mBuff = newBuff;
			}
		}
