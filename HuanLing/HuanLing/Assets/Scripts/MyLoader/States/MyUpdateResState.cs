...
{
	class MyUpdateResState : GameUpdateState
	{
		...
		public void Enter(LoadUI ui, object preData)
		{
			...
			{
				//尝试获取内部资源
				MyAssetsFileMgr.me.InitInner(InnerInited);
			}
		}

		private void InnerInited()
		{
		...
		private byte[] ReadWholeEntry(ZipInputStream zip, ZipEntry file)
		{
		...
		System.Collections.IEnumerator UnZipPack(INetResult result)
		{
			...
			while ((file = _Zip.GetNextEntry()) != null)
			{
				...
				if (file.Name == UpgradeFileName)
				{
					...
					_Rule = JsonUtility.FromJson<UpgradeRule>(str);
				}
			}
			file = null;
			_Zip.Close();
