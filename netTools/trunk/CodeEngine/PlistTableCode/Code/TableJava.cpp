...
STRING  CTableJava::GetRealType(const STRING& _sType ,STRING* defualtTypevalue)const
{
	...
	else if (sType == "BOOLEAN") { if(NULL != defualtTypevalue)*defualtTypevalue="false";return "boolean";}
	return "<error>";
}

STRING  CTableJava::GetRealTypeValue(const STRING& _sType,const STRING& sIdx)const
{
	std::string sType = __BB(_sType);
	if (sType == "FLOAT") 
	{
		string str = " Float.parseFloat(valuesList[";
		str += sIdx;
		str += "])";
		return str;
	}
	...
	if (sType == "INT") 
	{
		string str = " Integer.parseInt(valuesList[";
		str += sIdx;
		str += "])";
		return str;
	}
	if (sType == "SHORT") 
	{
		string str = " Short.parseShort(valuesList[";
		str += sIdx;
		str += "])";
		return str;
	}
	if (sType == "BYTE") 
	{
		string str = " Byte.parseByte(valuesList[";
		str += sIdx;
		str += "])";
		return str;
	}
...
BOOL CTableJava::CovertFileToCode( const STRING& rFile, STRING& error) const
{
	XMLReader_T _reader;
	if(FALSE == ReadTemplateXML(_reader,JAVA_CODE_TEMPLATE))
	{
		Model_Log::CashLog<const char*>("Read XML %s Error!!  Please Check XML",GetTemplatemanager(),Model_Log::CrashLog::LOG_ERROR);
		return FALSE;
	}

	STRING codeFileName = GetDir(FindXMLElement(_reader,"plist_namespace"));
	codeFileName += "Table_";
...

	{
		...
		STRING  enumList ="";
		INT k=0;
		for (LIstStruct_T::const_iterator it = listserial.begin();it != listserial.end();++it)
		{
			const Code_Serial& rData = *it;
			enumList += "public static final int ID_";
			enumList += __BB(rData.m_Name);
			enumList +=" = ";
			
			enumList += CovertIntToString(k++);
			enumList +=" ;\n";
			if(k==1)k++;
		}
		enumList += "public static final int MAX_RECORD = ";
		enumList += CovertIntToString(k++);
		enumList +=" ;\n";
...
BOOL CTableJava::WriteManager( const std::set<STRING>& rFile ) const
{
	XMLReader_T _reader;
...
STRING CTableJava::GetDir( const STRING& rPackage ) const
{
	STRING rootDir = GetDir();
	STRING package  =rPackage;
	//package com.gcgame.util;${N}
